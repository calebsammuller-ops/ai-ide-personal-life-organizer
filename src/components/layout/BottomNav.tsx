...
import { Brain, Network, Lightbulb, MessageCircle, Sparkles } from 'lucide-react'
        ...
        <div className="flex h-14 items-center justify-around px-2">
          {primaryNav.map((item) => {
            ...
            return (
              ...
              >
                {isActive && <div className="absolute inset-0 bg-primary/8" />}
                <div className={cn('relative z-10 p-1 transition-all duration-200', isActive && 'bg-primary/15')}>
                  <item.icon className={cn('h-4 w-4', isActive && 'drop-shadow-[0_0_6px_hsl(17,100%,56%)]')} />
                </div>
                <span className={cn(
