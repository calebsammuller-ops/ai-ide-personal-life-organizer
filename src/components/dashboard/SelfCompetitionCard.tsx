...
function DeltaTag({ value }: { value: number }) {
  if (value === 0) return <span className="text-[10px] text-muted-foreground/30">→ same</span>
  const isUp = value > 0
  return (
    <span className={`text-[10px] ${isUp ? 'text-primary/60' : 'text-destructive/60'}`}>
      {isUp ? '↑' : '↓'} {Math.abs(value)}%
    </span>
  )
}
  ...
  return (
    <Card className="rounded-lg border-border/40">
      <CardContent className="pt-3 pb-2">
        <p className="text-[10px] text-muted-foreground/30 font-medium mb-2">You vs You</p>
        <div className="space-y-1">
          ...
          {hasLastWeek && (
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-muted-foreground/30">Last week</span>
          ...
          {hasLastWeek && (
            <div className="flex items-center gap-3 pt-0.5">
              <div className="flex items-center gap-1">
                <span className="text-[10px] text-muted-foreground/30">ideas</span>
                <DeltaTag value={delta.notes} />
              </div>
              <div className="flex items-center gap-1">
                <span className="text-[10px] text-muted-foreground/30">connections</span>
